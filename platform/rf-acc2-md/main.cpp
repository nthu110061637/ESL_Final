...
public:
	...
	addr_t dma1_start_addr = 0x75000000;
	addr_t dma1_end_addr = 0x75001000;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > dma1_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	bus.ports[14] = new PortMapping(opt.dma1_start_addr, opt.dma1_end_addr);
	// connect TLM sockets
	iss_mem_if.isock.bind(bus.tsocks[0]);
	dbg_if.isock.bind(bus.tsocks[2]);

	PeripheralWriteConnector dma0_connector("SimpleDMA0-Connector"); 
	PeripheralWriteConnector dma1_connector("SimpleDMA1-Connector");  // to respect ISS bus locking
	dma0_connector.isock.bind(bus.tsocks[1]);

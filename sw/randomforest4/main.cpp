...
static char* const RandomForest2_START_ADDR = reinterpret_cast<char* const>(0x75000000);
static char* const RandomForest2_READ_ADDR  = reinterpret_cast<char* const>(0x75000004);
static char* const RandomForest3_START_ADDR = reinterpret_cast<char* const>(0x76000000);
static char* const RandomForest3_READ_ADDR  = reinterpret_cast<char* const>(0x76000004);
// DMA 
  ...
  for( int r = 0 ; r < NumData ; r++){
    for( int i = 0; i < condition + 1; i++ )
    {
      if(i<condition){
        ...
        buffer2[0] = (unsigned char)TestData[r][i];
        buffer3[0] = (unsigned char)TestData[r][i];
        write_data_to_ACC(RandomForest0_START_ADDR, buffer0, 4);
        write_data_to_ACC(RandomForest1_START_ADDR, buffer1, 4);
        write_data_to_ACC(RandomForest2_START_ADDR, buffer2, 4);
        write_data_to_ACC(RandomForest3_START_ADDR, buffer3, 4);
      }
      else{
        num0 = 1;
        num1 = 2;
        ...
        buffer2[0] = (unsigned char)num2;
        buffer3[0] = (unsigned char)num3;
        write_data_to_ACC(RandomForest0_START_ADDR, buffer0, 4);
        write_data_to_ACC(RandomForest1_START_ADDR, buffer1, 4);
        write_data_to_ACC(RandomForest2_START_ADDR, buffer2, 4);
        write_data_to_ACC(RandomForest3_START_ADDR, buffer3, 4);
    ...
    read_data_from_ACC(RandomForest0_READ_ADDR, buffer0, 4);
    read_data_from_ACC(RandomForest1_READ_ADDR, buffer1, 4);
    read_data_from_ACC(RandomForest2_READ_ADDR, buffer2, 4);
    read_data_from_ACC(RandomForest3_READ_ADDR, buffer3, 4);
    memcpy(data0.uc, buffer0, 4);
    memcpy(data1.uc, buffer1, 4);
    memcpy(data2.uc, buffer2, 4);
    memcpy(data3.uc, buffer3, 4);
    Result[r][0] = (int)data0.uc[0];
    Result[r][1] = (int)data1.uc[0];
    Result[r][2] = (int)data2.uc[0];
    Result[r][3] = (int)data3.uc[0];
  }
